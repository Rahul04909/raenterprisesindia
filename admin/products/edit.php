...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        if (isset($_FILES['brochure_file']) && $_FILES['brochure_file']['error'] == 0) {
             ...
             if (!file_exists($brochureDir)) mkdir($brochureDir, 0777, true);
        ...
        if (isset($_POST['delete_images'])) {
            foreach ($_POST['delete_images'] as $imgId) {
                // Optionally delete file from server here
        ...
        if (!empty($_POST['gallery_image_urls'])) {
            ...
            foreach ($urls as $url) {
                $url = trim($url);
                if ($url) {
                    $pdo->prepare("INSERT INTO product_images (product_id, image_path) VALUES (?, ?)")->execute([$id, $url]);
                }
            }
        }
        ...
        if (isset($_FILES['gallery_image_files'])) {
            ...
            for ($i = 0; $i < count($files['name']); $i++) {
                if ($files['error'][$i] == 0) {
                     ...
                     $newName = 'gallery-' . $id . '-' . $i . '-' . time() . '.' . $ext;
                     $uploadDir = '../../assets/uploads/products/';
                     if (!file_exists($uploadDir)) mkdir($uploadDir, 0777, true);
                     move_uploaded_file($files['tmp_name'][$i], $uploadDir . $newName);
                     $path = 'assets/uploads/products/' . $newName;
                     $pdo->prepare("INSERT INTO product_images (product_id, image_path) VALUES (?, ?)")->execute([$id, $path]);
                }
            }
        }
...
<head>
    ...
    <style>
        ...
        .gallery-item { position: relative; display: inline-block; margin: 5px; border: 1px solid #ccc; padding: 5px; }
        .gallery-item img { width: 100px; height: 100px; object-fit: cover; display: block; }
        .gallery-item label { display: block; margin-top: 5px; font-size: 12px; color: #d63638; cursor: pointer; }
        
        /* Specific Pricing Logic */
        #pricing_fields { display: <?php echo $product['is_price_enabled'] ? 'block' : 'none'; ?>; background: #f9f9f9; padding: 15px; border: 1px solid #eee; margin-top: 10px; }
        #price_msg { display: <?php echo $product['is_price_enabled'] ? 'none' : 'block'; ?>; color: #666; font-size: 13px; margin-top: 10px; }
    </style>
</head>
<body>
<div class="admin-wrapper">
    ...
    <div class="admin-content">
        ...
        <main class="admin-main">
            ...
            <?php if ($success): ?><div style="background:white; border-left:4px solid green; padding:15px; margin-bottom:20px;"><?php echo $success; ?> <a href="index.php">Back to List</a></div><?php endif; ?>
                ...
                <div class="form-layout">
                    ...
                    <div class="form-main">
                        <div class="card">
                           <div class="form-group">
                                ...
                                <input type="text" name="name" id="name" required value="<?php echo htmlspecialchars($product['name']); ?>" style="font-size: 1.2em; padding: 10px;">
                           </div>
                           <div class="form-group">
                                <label>Permalink (Slug)</label>
                                <input type="text" name="slug" id="slug" required value="<?php echo htmlspecialchars($product['slug']); ?>" style="background: #f0f0f1; color: #666;">
                           </div>
                        </div>
                        ...
                        <div id="tab-general" class="tab-content active">
                            <div class="card">
                                <div class="form-group">
                                    ...
                                    <textarea name="short_description" rows="3"><?php echo htmlspecialchars($product['short_description']); ?></textarea>
                                </div>
                                <div class="form-group">
                        ...
                        <div id="tab-spec" class="tab-content">
                            <div class="card">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="hsn" value="<?php echo htmlspecialchars($product['hsn']); ?>">
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <?php if($product['brochure_path']): ?>
                                        <div style="margin-bottom:5px;">
                                            <a href="../../<?php echo $product['brochure_path']; ?>" target="_blank" style="color:#2271b1;">View Current Brochure</a>
                                        </div>
                                    <?php endif; ?>
                        ...
                        <div id="tab-seo" class="tab-content">
                            <div class="card">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="meta_title" id="meta_title" value="<?php echo htmlspecialchars($product['meta_title']); ?>">
                                    <small><button type="button" onclick="generateSEO()">Auto-Generate</button></small>
                                </div>
                                <div class="form-group">
                                    <label>Meta Description</label>
                                    <textarea name="meta_description" id="meta_description" rows="2"><?php echo htmlspecialchars($product['meta_description']); ?></textarea>
                                </div>
                                <div class="form-group">
                                    <label>Meta Keywords</label>
                                    <input type="text" name="meta_keywords" value="<?php echo htmlspecialchars($product['meta_keywords']); ?>">
                                </div>
                                <div class="form-group">
                                    <label>Schema Markup (JSON-LD)</label>
                                    <textarea name="schema_markup" id="schema_markup" rows="8" style="font-family: monospace; font-size: 12px;"><?php echo htmlspecialchars($product['schema_markup']); ?></textarea>
                                </div>
                            </div>
                            <div class="card">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="og_title" value="<?php echo htmlspecialchars($product['og_title']); ?>">
                                </div>
                                <div class="form-group">
                                    <label>OG Description</label>
                                    <textarea name="og_description" rows="2"><?php echo htmlspecialchars($product['og_description']); ?></textarea>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="form-sidebar">
                        ...
                        <div class="card">
                            ...
                            <div class="form-group">
                                <label>Updated</label>
                                <div style="color:#666; font-size:12px;"><?php echo $product['updated_at']; ?></div>
                            </div>
                            <div style="text-align: right; margin-top: 10px;">
                                <button type="submit" class="button button-primary" style="background: #2271b1; color: white; padding: 10px 20px; width: 100%;">Update Product</button>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="brand_category_id" id="brand_cat_select">
                                    ...
                                    <?php foreach ($brand_categories as $bc): ?>
                                        <option value="<?php echo $bc['id']; ?>" data-brand="<?php echo $bc['brand_id']; ?>" <?php echo $bc['id'] == $product['brand_category_id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($bc['name']); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="card">
                            ...
                            <div class="toggle-switch">
                                <input type="checkbox" name="is_price_enabled" id="is_price_enabled" onchange="togglePricing()" <?php echo $product['is_price_enabled'] ? 'checked' : ''; ?>>
                                <label for="is_price_enabled">Enable Price/Stock Management</label>
                            </div>
                            <div id="pricing_fields">
                                <div class="form-group">
                                    <label>MRP</label>
                                    <input type="number" name="mrp" step="0.01" value="<?php echo $product['mrp']; ?>">
                                </div>
                                <div class="form-group">
                                    <label>Sales Price</label>
                                    <input type="number" name="sales_price" step="0.01" value="<?php echo $product['sales_price']; ?>">
                                </div>
                                <div class="form-group">
                            ...
                            <p id="price_msg">Product will act as "Price on Request" if disabled.</p>
                        </div>
                                ...
                                <?php if($product['featured_image']): ?>
                                    <div style="margin-bottom:10px;">
                                        <?php 
                                            ...
                                            if(strpos($featImg, 'http') !== 0) $featImg = '../../' . $featImg;
                                        ?>
                                        <img src="<?php echo htmlspecialchars($featImg); ?>" style="width:100%; height:auto; border:1px solid #ddd;">
                                    </div>
                                <?php endif; ?>
                                ...
                                <div style="margin-bottom:10px;">
                                    <?php foreach($gallery_images as $img): ?>
                                        <div class="gallery-item">
                                            <?php 
                                                $gImg = $img['image_path'];
                                                if(strpos($gImg, 'http') !== 0) $gImg = '../../' . $gImg;
                                            ?>
                                            <img src="<?php echo htmlspecialchars($gImg); ?>">
                                            <label>
                                                <input type="checkbox" name="delete_images[]" value="<?php echo $img['id']; ?>"> Remove
                                            </label>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
...
<script>
    ...
    document.getElementById('name').addEventListener('keyup', function() {
        let name = this.value;
        // Only update slug if it's empty or user explicitly wants to? 
        // Usually on Edit we don't auto-update slug unless requested to avoid breaking SEO.
        // Uncomment below if auto-update is desired on edit
        /*
